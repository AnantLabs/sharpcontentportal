var scp;	//should really make this m_scp... but want to treat like namespace

var scp_HIGHLIGHT_COLOR = '#9999FF';
...
__scp.prototype = 
{
	getVars: function()
	{
		...
		{
			this.vars = new Array();
			var oCtl = scp.dom.getById('__scpVariable');
			if (oCtl != null)
			{
	...
	{			
		...
		if (oCtl == null)
		{
			oCtl = scp.dom.createElement('INPUT');
			oCtl.type = 'hidden';
			oCtl.id = '__scpVariable';
			scp.dom.appendChild(scp.dom.getByTagName("body")[0], oCtl);		
		}
		var sVals = '';
		var s;
		var re = eval('/"/g');
	...
	{
		var sPostBack = scp.getVar('__scp_postBack');
		var sData = '';
	...
	doDelay: function(sType, iTime, pFunc, oContext) 
	{
		...
		{
			this.delay[sType] = new scp.delayObject(pFunc, oContext, sType);
			//this.delay[sType].num = window.setTimeout(scp.dom.getObjMethRef(this.delay[sType], 'complete'), iTime);
			this.delay[sType].num = window.setTimeout(scp.createDelegate(this.delay[sType], this.delay[sType].complete), iTime);
		}
	},
	...
	{
		for (var i=0; i<__scp_m_aNamespaces.length; i++)
		{
			for (var iDep=0; iDep<__scp_m_aNamespaces[i].dependencies.length; iDep++)
			{
				if (__scp_m_aNamespaces[i].dependencies[iDep] == sPNS + (sPNS.length>0 ? '.': '') + sNS)
	...
	__scp.prototype.delayObject = function(pFunc, oContext, sType)
	{
		this.num = null;
		this.pfunc = pFunc;
	...
	__scp.prototype.delayObject.prototype =
	{
		complete: function()
		{
			scp.delay[this.type] = null;
			this.pfunc(this.context);
		}
	}

	__scp.prototype.ScriptRequest = function(sSrc, sText, fCallBack)
	{
		this.ctl = null;
		this.src = null;
		this.text = null;
	...
	__scp.prototype.ScriptRequest.prototype = 
	{
		load: function()
		{
			this.status = 'loading';
			...
			var oHeads = scp.dom.getByTagName('HEAD');
			if (oHeads)
...
scp_dom.prototype =
{
		...
		{
			if (scp.dom.browser.isType(scp.dom.browser.InternetExplorer) == false)
			{
				var sName = sType.substring(2);
				oCtl.addEventListener(sName, function(evt) {scp.dom.event = new scp.dom.eventObject(evt, evt.target); return fHandler();}, false);
			}
			else
				oCtl.attachEvent(sType, function() {scp.dom.event = new scp.dom.eventObject(window.event, window.event.srcElement); return fHandler();});
			return true;
		},		
		...
		collapseElement: function(oCtl, iNum, pCallBack) 
		{
			...
			scp.dom.__collapseElement(oContext);
		},
		...
		__collapseElement: function(oContext) 
		{
			...
			if (oCtl.offsetHeight - (iStep*2) > 0)
			{
				...
				scp.doDelay(oCtl.id + 'col', 10, scp.dom.__collapseElement, oContext);
			}
			else
			{
				oCtl.style.display = 'none';
		...
		expandElement: function(oCtl, iNum, pCallBack) 
		{
			...
			oContext.ctl = oCtl;
			oContext.pfunc = pCallBack;
			scp.dom.__expandElement(oContext);
		},
		...
		__expandElement: function(oContext) 
		{
			...
			{
				oCtl.style.height = oCtl.offsetHeight + iStep;
				scp.doDelay(oCtl.id + 'exp', 10, scp.dom.__expandElement, oContext);
			}
			else
			{
				oCtl.style.overflow = '';
		...
		getById: function(sID, oCtl)
		{
			if (oCtl == null)
				oCtl = document;
			if (oCtl.getElementById) //(scp.dom.browser.isType(scp.dom.browser.InternetExplorer) == false)
		...
		getByTagName: function(sTag, oCtl)
		{
			if (oCtl == null)
				oCtl = document;
			if (oCtl.getElementsByTagName) //(scp.dom.browser.type == scp.dom.browser.InternetExplorer)
		...
		addSafeHandler: function(oDOM, sEvent, oObj, sMethod)
		{
			...
			if (scp.dom.browser.isType(scp.dom.browser.InternetExplorer))	//handle IE memory leaks with closures
			{
				if (this.__leakEvts.length == 0)
					scp.dom.attachEvent(window, 'onunload', scp.dom.destroyHandlers);

				this.__leakEvts[this.__leakEvts.length] = new scp.dom.__leakEvt(sEvent, oDOM, oDOM[sEvent]);
			}
		},
		
		destroyHandlers: function()	//handle IE memory leaks with closures
		{
			var iCount = scp.dom.__leakEvts.length-1;
			for (var i=iCount; i>=0; i--)
			{
				var oEvt = scp.dom.__leakEvts[i];
				oEvt.ctl.detachEvent(oEvt.name, oEvt.ptr);
				oEvt.ctl[oEvt.name] = null;
				scp.dom.__leakEvts.length = scp.dom.__leakEvts.length - 1;
			}
		},
		...
		getScript: function(sSrc)
		{
			...
			var oScripts = (document.scripts != null ? document.scripts : scp.dom.getByTagName('SCRIPT'));
			var oScripts = scp.dom.getByTagName('SCRIPT');	//safari has document.scripts
		...
		{
			var oThisScript = scp.dom.getScript('scp.js');
			if (oThisScript)
				return oThisScript.src.replace('scp.js', '');
			return '';
		},
		...
		loadScript: function(sSrc, sText, callBack)
		{
			...
			var oSR = new scp.ScriptRequest(sSrc, sText, callBack);
			if (sFile)
		...
		{
			var sFile = this.scriptFile(sSrc);
			if (this.scripts[sFile] && scp.dom.scripts[sFile].status != 'complete')
				scp.dom.scripts[sFile].complete();		
		},
		...
		getElementPostString: function(oCtl)
		{
			...
			if (sTagName == 'input') 
			{
				...
				if (sType == 'text' || sType == 'password' || sType == 'hidden' || ((sType == 'checkbox' || sType == 'radio') && oCtl.checked)) 
					return oCtl.name + '=' + scp.encode(oCtl.value) + '&';
			}
			else if (sTagName == 'select') 
			{
				for (var i=0; i<oCtl.options.length; i++) 
				{
					if (oCtl.options[i].selected) 
						return oCtl.name + '=' + scp.encode(oCtl.options[i].value) + '&';
				}
			}
			else if (sTagName == 'textarea') 
					return oCtl.name + '=' + scp.encode(oCtl.value) + '&';
		...
		scp_dom.prototype.eventObject = function(e, srcElement)
		{
			this.object = e;
		...
		//--- scp.dom.browser
		scp_dom.prototype.browserObject = function()
		{
...
scp_dom.prototype.browserObject.prototype =
{
		toString: function()
		{
		...
		isType: function()
		{
			...
			{
				if (scp.dom.browser.type == arguments[i])
					return true;
			}
			return false;
		}
	...
	//--- scp.controls - not enough here to justify separate js file	
	function scp_controls()
	{
		this.pns = 'scp';
		this.ns = 'controls';
		this.dependencies = 'scp,scp.dom'.split(',');	//,scp.xml - removed 10/17/06
		this.isLoaded = false;
...
scp_controls.prototype = 
{
	dependenciesLoaded: function()
	{
		return (typeof(scp) != 'undefined' && typeof(scp.dom) != 'undefined');	//removed && typeof(scp.xml) != 'undefined' - 10/17/06
	},
		...
		if (this.isLoaded == false)
		{
			...
			{				
				if (typeof(scp_control) != 'undefined')
					scp.extend(scp_controls.prototype, new scp_control);
	...
	scp_controls.prototype.scpNode = function(oNode)
	{
		if (oNode != null)
		{
			this.node = oNode; 
...
scp_controls.prototype.scpNode.prototype = 
{
...
//--- End scp.controls
	...
	scp_util.prototype.dependenciesLoaded = function()
	{
		return (typeof(scp) != 'undefined' && typeof(scp.dom) != 'undefined');
	}

	scp_util.prototype.loadNamespace = function()
	{
		if (this.isLoaded == false)
		{
			if (this.dependenciesLoaded())
			{				
				if (typeof(scp_utility) != 'undefined')
					scp.extend(scp_util.prototype, new scp_utility);
...
//load namespaces
__scp_m_aNamespaces[__scp_m_aNamespaces.length] = new scp_util();
__scp_m_aNamespaces[__scp_m_aNamespaces.length] = new scp_controls();
__scp_m_aNamespaces[__scp_m_aNamespaces.length] = new scp_dom();
__scp_m_aNamespaces[__scp_m_aNamespaces.length] = new __scp();
